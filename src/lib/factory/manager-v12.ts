...
function researchSignals(input: PriorityAwareFactoryManagerInput) {
  const live = (input.liveResearch || {}) as any
  return Array.isArray(live.signals) ? live.signals as DeepResearchSignalV12[] : []
}

function requestedSpecialized(report: FactoryManagerV10Report, input: PriorityAwareFactoryManagerInput) {
  const live = (input.liveResearch || {}) as any
  const explicit = Array.isArray(live?.profile?.specializedCapabilities)
    ? live.profile.specializedCapabilities.filter((value: unknown): value is string => typeof value === 'string' && Boolean(value.trim()))
    : []
  if (explicit.length) return [...new Set(explicit)]
  return report.customerBrief.capabilities.filter((capability) => !GENERIC.has(capability.toLowerCase()))
}

function githubSignals(input: PriorityAwareFactoryManagerInput) {
...
function makePlan(
  ...
  const inspection = Math.round(repos.reduce((sum, repo) => sum + numberValue(signalByName.get(repo.fullName.toLowerCase())?.inspection?.inspectionScore, repo.evidenceStrength), 0) / repos.length)
  ...
  const fit = Math.round(clamp(coverage * 0.42 + relevance * 0.24 + inspection * 0.18 + maintenance * 0.08 + license * 0.08 - complexityPenalty, 15, 97))
  const confidence = Math.round(clamp(fit * 0.48 + inspection * 0.28 + relevance * 0.16 + Math.min(100, repos.length * 16 + covered.length * 7) * 0.08, 20, 96))
  const integrationComplexity: CustomerComposition['integrationComplexity'] = repos.length <= 1 && runtimeCount <= 1 ? 'Low' : repos.length <= 3 && runtimeCount <= 2 ? 'Medium' : 'High'
...
function buildPlans(repos: RankedRepo[], requested: string[], idea: string, signalByName: Map<string, DeepResearchSignalV12>) {
  const candidates = [
...
function implementationPrompt(idea: string, plan: CustomerComposition | undefined, report: FactoryManagerV12Report, signalByName: Map<string, DeepResearchSignalV12>) {
...
export function createFactoryManagerV12Report(input: PriorityAwareFactoryManagerInput): FactoryManagerV12Report {
  ...
  const repoSignals = githubSignals(input).filter((signal) => repos.some((repo) => repo.fullName.toLowerCase() === text(signal.repository?.fullName).toLowerCase()))
  const averageInspection = repoSignals.length ? Math.round(repoSignals.reduce((sum, signal) => sum + numberValue(signal.inspection?.inspectionScore), 0) / repoSignals.length) : 0
  const averageRepositoryRelevance = repoSignals.length ? Math.round(repoSignals.reduce((sum, signal) => sum + numberValue(signal.relevance) * 100, 0) / repoSignals.length) : 0
  ...
  const qualityScore = Math.round(clamp(researchCompleteness * 0.25 + capabilityCoverage * 0.35 + averageInspection * 0.25 + averageRepositoryRelevance * 0.15 + evidenceBonus))
  const relevantSignals = researchSignals(input).filter((signal) => signal.kind !== 'github-source-proof' && numberValue(signal.relevance) >= 0.68)
  ...
  const band: FactoryManagerV10Report['recommendationQuality']['band'] = gatePassed ? 'High' : qualityScore >= 75 ? 'Medium' : 'Needs more evidence'
  ...
  const result: FactoryManagerV12Report = {
    ...
    managerVerdict: {
      ...base.managerVerdict,
      decision: !gatePassed ? 'RESEARCH_MORE' : recommended && recommended.estimatedFit >= 88 && recommended.confidence >= 85 && recommended.capabilityCoverage >= 85 ? 'GO' : 'GO_WITH_GUARDS',
    ...
    sourceIntelligence: {
      ...
      topSignals: researchSignals(input).filter((signal) => numberValue(signal.relevance) >= 0.62).sort((a, b) => numberValue(b.relevance) - numberValue(a.relevance)).slice(0, 24) as any,
