  ...
  if (recommended) {
    ...
    report.managerVerdict = {
      ...
      summary: weakEvidence
        ? `The ${recommended.customerTitle} plan currently looks most aligned with the customer's “${desiredEffort(input.customerContext?.priority)}” priority, but the evidence is not yet strong enough for a confident build recommendation. Research more before locking sources.`
