...
export type PriorityAwareFactoryManagerInput = FactoryManagerV10Input & {
...
function reorderPlans(report: FactoryManagerV10Report, input: PriorityAwareFactoryManagerInput) {
  ...
  const chosen = explicit || report.compositionSuggestions.find((plan) => plan.effort === wanted)
...
export function createPriorityAwareFactoryManagerV10Report(input: PriorityAwareFactoryManagerInput): FactoryManagerV10Report {
  ...
  if (recommended) {
    ...
    report.managerVerdict = {
      ...
      confidence: weakEvidence ? Math.min(recommended.confidence, 64) : recommended.confidence,
      summary: weakEvidence
        ? `The ${recommended.customerTitle} plan currently looks most aligned, but the evidence is not yet strong enough for a confident build recommendation. Research more before locking sources.`
        : explicitSelection
          ? `The customer's selected “${recommended.customerTitle}” plan has been re-checked at ${recommended.estimatedFit}% implementation fit. The score combines product relevance, capability coverage, maintenance, license safety and integration complexity.`
          : `${recommended.customerTitle} is the strongest plan for the customer's “${desiredEffort(input.customerContext?.priority)}” priority at ${recommended.estimatedFit}% implementation fit. The score combines product relevance, capability coverage, maintenance, license safety and integration complexity.`,
      reasons: recommended.whyThisCombination,
      riskFlags: [
        ...report.managerVerdict.riskFlags,
        ...(weakEvidence ? ['Evidence quality is below the recommendation gate; do not treat the current plan as build-ready yet.'] : []),
      ],
    }
  }
