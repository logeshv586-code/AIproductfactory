...
type ResearchResponse = LiveResearch & {
  ...
  summary?: LiveResearch['summary'] & {
    ...
    averageRunnable?: number
  }
  architecturePatterns?: string[]
  researchRuntime?: {
    mode?: string
    githubAuthenticated?: boolean
    suppliedSeedRepositories?: number
    expertSeedRepositories?: string[]
    effectiveSeedRepositories?: string[]
    githubRequestBudget?: string
    guidance?: string
  }
}
...
export default function FactoryStudioV11() {
  ...
  const hasBuildPlans = Boolean(report?.compositionSuggestions.length)
  ...
  async function analyze() {
    ...
    try {
      ...
      const publicGithubLimited = research.researchRuntime?.githubAuthenticated === false && Number(research.summary?.githubCandidates || 0) === 0
      setStatus(manager.report.managerVerdict.decision === 'RESEARCH_MORE'
        ? publicGithubLimited
          ? 'GitHub evidence access is public-rate-limited. Configure GITHUB_TOKEN and analyze again before treating zero qualified repositories as a strong conclusion.'
          : 'AI needs stronger evidence before it should recommend a build. Review the expert research gaps or analyze again.'
        : 'Analysis complete. Review the expert evidence, architecture and source-qualified plan.')
    } catch (cause) {
  ...
  return <main className="min-h-screen bg-[#f6f8fc] text-slate-800">
    ...
    <div className="relative mx-auto max-w-[1480px] px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
      <header className="mb-5 flex flex-col gap-4 rounded-[26px] border border-white/90 bg-white/85 p-4 shadow-[0_22px_70px_-38px_rgba(15,23,42,0.35)] backdrop-blur-xl sm:p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br from-blue-600 to-violet-600 text-white shadow-lg shadow-blue-200/80"><WandSparkles className="h-5 w-5" /></div><div><div className="flex items-center gap-2"><h1 className="text-lg font-semibold tracking-tight text-slate-950">AI Product Factory</h1><Pill tone="purple">Expert Studio</Pill></div><p className="mt-0.5 text-xs text-slate-500">Plain-language idea → high-detail research → evidence-backed architecture → verified build.</p></div></div>
        <div className="flex flex-wrap items-center gap-2"><div className="hidden"><button onClick={() => setExpertMode(false)}>Simple</button><button onClick={() => setExpertMode(true)}>Expert</button></div><Pill tone="purple">Expert mode always on</Pill><span className="hidden rounded-xl border border-emerald-100 bg-emerald-50 px-3 py-2 text-xs font-medium text-emerald-700 sm:inline-flex">● Source proof required before recommendation</span></div>
      </header>
      ...
      <section className="mt-5 overflow-hidden rounded-[32px] border border-slate-200/80 bg-white shadow-[0_30px_90px_-46px_rgba(30,64,175,0.38)]">
        <div className="grid xl:grid-cols-[1.18fr_0.82fr]">
          <div className="p-6 sm:p-8 lg:p-10">
            <Pill tone="purple">Expert workflow · no coding required</Pill>
            <h2 className="mt-5 max-w-4xl text-3xl font-semibold tracking-[-0.04em] text-slate-950 sm:text-4xl lg:text-[52px] lg:leading-[1.04]">Describe the outcome. <span className="bg-gradient-to-r from-blue-600 to-violet-600 bg-clip-text text-transparent">AI builds the technical case.</span></h2>
            <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-600 sm:text-base">The factory converts your idea into a capability graph, researches current implementations, inspects source evidence, rejects weak matches and exposes architecture-level reasoning before anything can be approved.</p>
          ...
          <div className="border-t border-slate-200 bg-gradient-to-br from-slate-50 via-blue-50/50 to-violet-50/50 p-6 sm:p-8 xl:border-l xl:border-t-0 xl:p-10"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-slate-500"><Stars className="h-4 w-4 text-blue-600" /> Expert analysis pipeline</div><h3 className="mt-3 text-2xl font-semibold tracking-tight text-slate-950">High-level architecture first. Raw evidence stays inspectable.</h3><div className="mt-7 space-y-3">{['Understand the goal, users and operating constraints','Build a specialized capability graph','Run capability-specific repository discovery','Inspect README, source and runnable-code evidence','Compose and score architecture alternatives','Lock only the exact plan and sources you approve'].map((text, index) => <div key={text} className="flex items-center gap-3 rounded-2xl border border-white/80 bg-white/80 p-4 shadow-sm"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-slate-950 text-xs font-semibold text-white">{String(index + 1).padStart(2, '0')}</span><span className="text-sm font-medium text-slate-700">{text}</span></div>)}</div><div className="mt-6 rounded-2xl border border-blue-100 bg-white p-4 shadow-sm"><div className="flex items-center gap-2 text-sm font-semibold text-slate-900"><LockKeyhole className="h-4 w-4 text-blue-600" /> Evidence-gated control</div><p className="mt-2 text-xs leading-5 text-slate-500">The model can reason with DeepSeek, OpenAI, Claude, Gemini or local AI, but repository qualification remains source-driven and cannot be overridden by model confidence alone.</p></div></div>
        </div>
      </section>
      ...
      {report && <div className="mt-7 space-y-6">
        ...
        {expertMode && <section className="rounded-[28px] border border-slate-200 bg-white shadow-sm"><details open><summary className="flex cursor-pointer list-none items-center justify-between p-5 sm:p-6"><div><div className="text-[10px] font-bold uppercase tracking-[0.18em] text-violet-600">Expert research intelligence</div><h3 className="mt-1 text-lg font-semibold text-slate-950">High-level research state, source proof and developer handoff</h3></div><ChevronDown className="h-5 w-5 text-slate-400" /></summary><div className="border-t border-slate-100 p-5 sm:p-6">
        <div className={cn('mb-5 rounded-[22px] border p-4 sm:p-5', liveResearch?.researchRuntime?.githubAuthenticated === false ? 'border-amber-200 bg-amber-50' : 'border-emerald-200 bg-emerald-50')}><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><div className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-500">Research operating state</div><div className="mt-1 text-sm font-semibold text-slate-950">{liveResearch?.researchRuntime?.githubAuthenticated === false ? 'Public GitHub research · rate-limited' : 'Authenticated GitHub deep research'}</div><p className="mt-2 max-w-4xl text-xs leading-5 text-slate-600">{liveResearch?.researchRuntime?.guidance || 'Source-qualified repositories must pass capability, source-inspection and runnable-code gates.'}</p></div><div className="flex flex-wrap gap-2"><Pill tone={liveResearch?.researchRuntime?.githubAuthenticated === false ? 'amber' : 'green'}>{liveResearch?.researchRuntime?.githubRequestBudget || 'source-proof'}</Pill><Pill tone="purple">{liveResearch?.researchRuntime?.mode || 'expert-source-proof'}</Pill></div></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-xl border border-white/80 bg-white/80 p-3"><div className="text-[10px] uppercase tracking-wide text-slate-400">Research completeness</div><div className="mt-1 text-xl font-semibold text-slate-950">{liveResearch?.summary?.researchCompleteness ?? 0}%</div></div><div className="rounded-xl border border-white/80 bg-white/80 p-3"><div className="text-[10px] uppercase tracking-wide text-slate-400">Capability coverage</div><div className="mt-1 text-xl font-semibold text-slate-950">{liveResearch?.summary?.capabilityCoverage ?? 0}%</div></div><div className="rounded-xl border border-white/80 bg-white/80 p-3"><div className="text-[10px] uppercase tracking-wide text-slate-400">Repositories discovered</div><div className="mt-1 text-xl font-semibold text-slate-950">{liveResearch?.summary?.repositoriesDiscovered ?? 0}</div></div><div className="rounded-xl border border-white/80 bg-white/80 p-3"><div className="text-[10px] uppercase tracking-wide text-slate-400">Repositories inspected</div><div className="mt-1 text-xl font-semibold text-slate-950">{liveResearch?.summary?.repositoriesInspected ?? 0}</div></div></div>{Boolean(liveResearch?.architecturePatterns?.length) && <div className="mt-4"><div className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400">Architecture patterns observed</div><div className="mt-2 flex flex-wrap gap-2">{liveResearch?.architecturePatterns?.slice(0, 10).map((pattern) => <Pill key={pattern} tone="blue">{pattern}</Pill>)}</div></div>}</div>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Relevant evidence" value={report.sourceIntelligence.signalCount} helper={`${report.sourceIntelligence.rejectedSignalCount} weak signals filtered.`} icon={Zap} /><Metric label="GitHub candidates" value={report.sourceIntelligence.githubCandidates} helper="Capability-matched candidates." icon={Github} /><Metric label="Qualified repos" value={report.recommendationQuality.repositoriesQualified} helper={`From ${report.recommendationQuality.repositoriesConsidered} considered.`} icon={PackageSearch} /><Metric label="Average relevance" value={`${report.sourceIntelligence.averageRelevance}%`} helper="Only allowed evidence." icon={BarChart3} /></div>
